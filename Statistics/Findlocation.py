...
def find_location_of_object(base_root0, namelist):
    f1all = []
    a1all = []
    f2all = []
    a2all = []
    ...
    for name_index in range(5):
        ...
        print(name_index)
        # 找peaks,记录声音的位置
        Sound_id, peakS_value, data_s = peaklocation(base_root, 'ch8.npy', 100, 400)  # 考虑到声音更短

        # 光颗粒物的位置，ch1--f, ch2--p
        peakLight_p, val_p, data_p = peaklocation(base_root, 'ch4.npy', 700, 400)
        peakLight_f, val_f, data_f = peaklocation(base_root, 'ch1.npy', 2000, 400)
